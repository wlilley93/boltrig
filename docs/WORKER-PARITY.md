...
Worker is Boltrig's task-first surface. Operator remains the advanced inspection,
configuration-history and rollback surface. This ledger prevents either client
from implying that a visible control is backed by authority it does not have.
...
Operator may remain the destination for raw event bodies, low-level
configuration history, rollback, and forensic diagnostics. A handoff to
Operator is not parity for an ordinary create, change, approve, revoke, export,
...
| Runs | Runs | cursor-page/search, cancel, execution/cost tree, durable subagent topology | scoped run, audit-tree and topology routes; raw event stream stays in Operator |
...
- 30 routes have an explicit non-UI classification: 17 service-native
  ...
  compatibility route and 7 intentionally Operator-only routes. The
  Operator-only group contains six raw
  configuration/history/rollback/credential controls and the raw run-event
  stream. Moving those seven into Worker would require a typed configuration
  apply contract or a bounded redacted event projection; embedding their
  current raw payloads would weaken the Worker/Operator authority split.
...
Operator or infrastructure boundary).
...
These are advanced inspection or recovery controls. Worker links to Operator
without copying this authority into a second implementation.
...
- serving consumers for the remaining parsed-only policy fields and complete
  ...
  rule authoring remains in Operator until Boltrig defines a canonical
...
separately authorized Operator run surface. Cursor frames report when the
