...
- HOLDS: the case file's citator claim is substantially correct. I read all thirteen orders in the repository's `.vjs/orders/` and grepped them for parity, store contract, conditional write, CAS, idempotency, exactly-once and double-execution; nothing is on all fours. Nothing in the applicable lawpack judgments (five opinions, all VJS-level constitutional or Opbox matters) touches it either.
