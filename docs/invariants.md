...
| **SEC-WRK-15** | Worker creates canonical internal work and assigns or unassigns it only through `control.work.*`; status and parent changes use the same dispatcher and high-consequence gate. Writes are keys-only audited and never imply source-system writeback. | `tests/security/test_work_lifecycle.py::test_work_create_assign_status_and_audit_are_canonical` |
