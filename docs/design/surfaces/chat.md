...
4. `attach(key)`: re-subscribes via `streamRunEvents(session.runId, onEvent, {follow:true})` (client.ts:752-796 -> `GET /v1/runs/{run_id}/events?follow=1`, app.py:534-558). The relay **replays its backlog from the start** then follows, so on attach the session's `events` array is **replaced wholesale** by the replayed stream - no dedupe heuristics. The backlog window remains bounded at 500 events. Development uses the in-process backend; production composition requires the shared Redis Streams backend. A 404 `unknown_run` or a replay that ends with no `message_end` falls back to "Refresh transcript" (fetch persisted state). Used by: StoppedNotice "Watch again", stream-drop recovery, and post-remount recovery of a session whose reader died.
