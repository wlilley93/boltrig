...
3. Submit: `POST /v1/invoke {noun:"control", verb:"control.capability.upsert", params:{name, runtime, supported_skills, max_depth, is_ephemeral, cost_tier, model_endpoint, vision_model_endpoint}}` (control_plane.py:64-76; app.py:259-283). Blank endpoint fields inherit the governed main API key by default; image turns use the optional main vision key when configured. The Agents tab can instead bind one endpoint advertising both `text` and `vision`, or explicit text and vision endpoints. Explicit bindings are per-agent overrides and retain their own model/host/provider while the kernel supplies the scoped credential. Busy label "Requesting..." (X6 fix). Never autosave (P16 hard rule: autosaving a governed verb spams the approvals inbox).
