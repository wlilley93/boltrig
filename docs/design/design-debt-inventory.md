...
- **W3. `source` enum exposed raw** - :1070-1082 (`precreated | generated | learned`): internal provenance vocabulary a human author has no reason to set. Replacement: default to precreated, move behind Advanced with a plain-language gloss, or set server-side.
