...
- **Scope:** the standalone Boltrig repo, the **pi runtime lane only**. The
...
- **Supersedes:** the Hermes-Agent-based assumptions in the prior Opbox full
  architecture spec do not apply to Boltrig and are not carried forward.
...
- The fleet already has a runtime named `hermes` (`boltrig/fleet/runtime.py`,
...
- **2.1 The kernel** - `boltrig/kernel/dispatch.py` runs every verb through the
...
- **2.2 The doctrine source** - the chokepoint/capability/audit discipline is
  ...
  source); Boltrig conforms, does not author. `scripts/check_invariants.py` +
...
- **3.1 No cross-turn conversation continuity (FOUNDATIONAL, fix first).**
  `boltrig/fleet/chat.py` `handle_turn` mints a brand new `run_id` and `WorkItem`
  ...
  upstream of them.)
- **3.2 No model-cost routing or caching.** `boltrig/fleet/model_router.py` is a
...
- **4.1 Session continuity (addresses 3.1).** Two paths: (a) **native Python,
  ...
  (`boltrig/store/base.py`), consistent with P1/P7; no new runtime. (b) Rivet
...
## 5. The invariant, restated in Boltrig's terms
...
- Whether `boltrig/identity` (auth/rbac/delegation/provisioning/tokens) maps
...
- Whether richer Hatchet workflow defs (retry/idempotency beyond the step-boundary
  seam in `boltrig/fleet/workers.py`) exist at the deployment layer or need
