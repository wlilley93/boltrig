# 0002 - Boltrig consolidation: conditioned standalone (VJS County Court)
...
Boltrig implements the same chokepoint + noun/verb + capability + audit doctrine
...
**Conditioned standalone.** Boltrig may persist as its own, independently-cloneable
...
| D4 | Track (never compete with) the doctrine's unified capability primitive | ARCHITECTURE note on `boltrig/models/grants.py`; tracked obligation |
