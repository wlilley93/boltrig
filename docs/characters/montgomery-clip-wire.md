...
FrameGraphStage iframes the player (loopback :8902) and speaks `postMessage`
...
| `clip:position` | `{hub}`                          | Walk him to a position (H1 desk, H3 fireplace, H4 window). H2 (beyond the far end of the table) and H5 (seated) were retired 2026-08-25 and are refused. |
...
{"type": "clip:state", "character": "General Montgomery",
 ...
 "positions": ["H1","H3","H4"], "talkBase": ["H1","H3","H4"],
 "entered": true, "speechHeld": false}
```

## The emotion contract

`entered` is false while he is still walking in through the doors: the
player HOLDS his first line for the six seconds that walk takes, so a reply
lands with him at the desk rather than halfway across the room.
