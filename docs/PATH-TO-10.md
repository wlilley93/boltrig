...
| Field | Value |
|---|---|
| Branch / SHA | `refactor/codex-thin-orchestration` / `9cc26f0` (clean, pushed to origin) |
| Working tree | Clean. Since the prior snapshot: slice 1 closed (`db43e59`), migration 0026 landed (`632fe76`), slice 2 landed (`273375c`), migration 0027 landed (`5ed7775`), and the durable root-decision adapter landed (`9cc26f0`). |
| Last verification | This session re-ran the Python legs only (all changes are Python application/store-layer): `make check` (offline) 1590 passed / 70 skipped, and `make python-quality` (real PostgreSQL + coverage) 1652 passed / 12 skipped, 83.83% total coverage. strict mypy (117 files), Ruff, architecture inward-only, structure ratchets, codex protocol pin; migration parity green at head 0027; invariant debt 0 (282 declared / 282 bound). UI/site/Playwright/compose/security legs were not re-run; they are unaffected by these changes and retain the prior 632fe76 snapshot. |
| Migration head | `0027_root_engine_decisions` (landed `5ed7775`). Migrations 0028-0031 (grant leases, capability attestations + assignment pins, approval/effect receipts) remain unwritten; in-memory adapters exist, PostgreSQL migrations and adapters do not (except root-decisions, which now has both). |
...
| 3 | PostgreSQL execution ledger migrations 0026-0028 | in progress | 0026 (execution ledger) and 0027 (root decisions) landed. APPLIED must prove a same-transaction aggregate mutation; deadlock lock-order tests; exact command-to-row transition proof for the execution-ledger command/event tables. Then 0030 (immutable capability attestations + assignment pins) and 0031 (approval/effect receipts). |
