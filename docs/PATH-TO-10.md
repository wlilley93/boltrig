...
| Last verification | This session re-ran the Python legs only (all changes are Python application/store-layer): `make check` (offline) 1600 passed / 135 skipped, and `make python-quality` (real PostgreSQL + coverage) 1723 passed / 12 skipped, 84.29% total coverage. strict mypy (117 files), Ruff, architecture inward-only, structure ratchets (no exemptions added), codex protocol pin; migration parity green at head 0031 (4 passed); invariant debt 0. The execution-ledger advisory lock was mutation-tested rather than trusted: removing it fails the concurrent compare-and-swap contract, restoring it passes. UI/site/Playwright/compose/security legs were not re-run; they are unaffected by these changes and retain the prior 632fe76 snapshot. |
| Migration head | `0031_execution_ledger_fidelity` (landed `ed021be`). Approval/effect receipts is now **0032** and remains unwritten. PostgreSQL: root-decisions, grant-leases, model-proxy-grants, capability-attestation sets, and the execution ledger now each have schema + durable adapter, so every in-memory store in this slice has a durable counterpart proven by one shared contract. **Capability-attestation caveat (still open):** the immutable set store is complete and self-contained, but binding the pin onto the `ExecutionAssignment` record (the pin is "persisted with an assignment") and writing sets during assignment creation is **not** done. It was deferred to the execution-ledger leg on the grounds that that is where the assignment is persisted; that leg has now landed without it, because it added no field to `ExecutionAssignment` and a durable adapter cannot bind a pin the model does not carry. It needs a model change plus migration 0032's assignment-pin column, and is tracked as slice-3/5 work, not as done. |
