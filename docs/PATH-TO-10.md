...
| Knowledge compiler | Kernel ledger plus Cognee's rebuildable enrichment projection | Live Cognee model configuration, degradation checks, and erasure verification. |
| Codex runtime and stack tools | Trusted per-cell Codex boundary, exact model/tool grants, and deployment receipts | Pinned Codex binary and schema evidence, constrained network/process posture, streaming/degradation checks, and proof that no ambient credentials cross the cell boundary. |
...
| Field | Value |
|---|---|
| Branch / SHA | `refactor/codex-thin-orchestration` / `61c65df` (clean, pushed to origin). Advances per landing; this row tracks the most recent code SHA, not a frozen point. |
| Working tree | Clean. Slices 0, 1, 2, 4 landed and slice 3's three proofs closed (see the slice table). Since the attestation-pin landing (`2ae9dd4`): a path-to-10 audit (10 read-only seats over the seven axes, adversarially verified) surfaced defects that were fixed and pushed, and the durable ledger admission service (`AssignmentAdmission`, `6abe103`) landed. Security/reliability fixes this session: SEC-164/165 pump principal capping (`929a274`), SEC-166 cancellation barrier (`1d7c86a`), SEC-167 MCP credential seam (`33af0b4`), SEC-168 whole-chain audit verification + FR-OPS-05 bounded `/healthz` (`61c65df`). |
| Last verification | `make check` (offline) 1663 passed / 150 skipped; `make python-quality` (real PostgreSQL + coverage) 1801 passed / 12 skipped, 84.43% total coverage; migration parity green at head 0032 (4 passed). strict mypy (119 files), Ruff, structure ratchets (no exemptions added, several tightened), codex protocol pin, invariant debt 0; architecture inward-only scans 68 files (it polices `models -> fleet`, previously unpoliced). Every security- and reliability-critical fix this session was mutation-tested rather than trusted green, and the mutation was confirmed to actually apply first (a mutation that silently fails to match its anchor gives a vacuous green). Recurring finding worth keeping: a "caller-cannot-supply-X" property holds as a SIGNATURE assertion (`inspect.signature`), not a value assertion; a bypass seam that merely defaults to the derived value passes every value test. UI/site/Playwright/compose legs were not re-run; they retain the prior `632fe76` snapshot. |
...
| 0 | Capability-attestation binding | landed (`8459e09`) | Attestation can only reject, never grant. Persistence LANDED (the insert-once set store, migration 0030, `49d0cc9`) and the pin is now bound to the assignment (migration 0032, `2ae9dd4`). Still open: writing sets during assignment creation is built (`AssignmentAdmission`, `6abe103`) but unwired (slice 5), and approval receipts (0033) are unwritten. |
| 1 | Raw-success-quarantined App Server phase execution + result projection | landed (`db43e59`) | Terminal-first / event-arbitration / sole-reader races landed and covered by a dedicated hardening suite (`test_codex_runtime_lifecycle_hardening.py`, 11 tests as collected today; do not read the historical "100" as current). Phase-result char-vs-byte budget re-derived so every schema-valid result fits the wire (24,976 B worst case, ~24% headroom), schema digest re-pinned, budget invariant un-xfailed; peer-payload retention invariant pinned (`3aa14c7`). |
...
| 6 | Legacy runtime removal | complete | Herdr, OpenCode, Rivet and provider-native agent runtimes, plugins, binaries, state volumes, readiness probes and revival flags are removed. Stale names fail closed; Codex is the only model-backed runtime. |
...
3. **Event winning over a terminal failure - confirmed landed.** Terminal-first
   ...
   `test_codex_runtime_lifecycle_hardening.py` (11 hardening tests green; the earlier "100" was a stale count, corrected 2026-07-17 against the collected total).
