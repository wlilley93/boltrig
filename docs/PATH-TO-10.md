...
| Field | Value |
|---|---|
| Branch / SHA | `refactor/codex-thin-orchestration` / `49d0cc9` (clean, pushed to origin) |
| Working tree | Clean. Since the prior snapshot: slice 1 closed (`db43e59`), migration 0026 landed (`632fe76`), slice 2 landed (`273375c`), migration 0027 landed (`5ed7775`), durable root-decision adapter landed (`9cc26f0`), the grant-lease schema landed (`4b85d23`), the durable grant-lease Postgres adapter landed (`b72f37e`), migration 0029 plus the durable model-proxy-grant Postgres adapter landed (`5470c67`), and migration 0030 plus the capability-attestation set store (memory + durable) landed (`49d0cc9`). |
| Last verification | This session re-ran the Python legs only (all changes are Python application/store-layer): `make check` (offline) 1600 passed / 127 skipped, and `make python-quality` (real PostgreSQL + coverage) 1715 passed / 12 skipped, 84.05% total coverage. strict mypy (117 files), Ruff, architecture inward-only, structure ratchets, codex protocol pin; migration parity green at head 0030 (4 passed); invariant debt 0. UI/site/Playwright/compose/security legs were not re-run; they are unaffected by these changes and retain the prior 632fe76 snapshot. |
| Migration head | `0030_capability_attestations` (landed `49d0cc9`). Migration 0031 (approval/effect receipts) remains unwritten. PostgreSQL: root-decisions, grant-leases, model-proxy-grants, and capability-attestation sets now each have schema + adapter. Execution-ledger has schema (0026) + in-memory adapter only, no durable adapter. **Capability-attestation caveat:** the immutable set store is complete and self-contained, but binding the pin onto the `ExecutionAssignment` record (the pin is "persisted with an assignment") and writing sets during assignment creation is deferred to the execution-ledger leg, where the assignment is persisted. |
