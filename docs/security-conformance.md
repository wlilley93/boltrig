...
| **INF** (infra) | **HARDENED + SEAM** | HARDENED (Round 17, SEC-64): INF-01 hardened containers - non-root images + read-only rootfs + cap_drop ALL + no-new-privileges + pids/mem limits on kernel/fleet (deploy-lint bound; runtime start needs a live docker host - ops). SEAM: INF-03 network segmentation (sandbox net exists, SEC-48), INF-05 least-priv DB role + RLS (next code+deploy round). |
