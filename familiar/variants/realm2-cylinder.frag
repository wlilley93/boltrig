...
void main(){
  ...
  float tempo   = (0.35 + 1.10*clamp(uArousal,0.0,1.0))*(1.0 - 0.45*fatigue);
  ...
  // a fifth of its old fixed rate - an animated idle, not a canned loop racing through its moves.
  // When boltrig's emotion relay is live, arousal/fatigue speed and slow the whole world directly.
  float t       = iTime*0.55*tempo;
  ...
  if (ga > 0.001) {
    ...
    else if (gid == 6) { gLean  += sin(iTime*10.0)*ga*0.3; }
    else if (gid == 7) { gSwell -= ga*0.08; gLean  -= ga*0.7; gChurn += ga*0.9; }
    else if (gid == 8) { gBright+= ga*0.35*(0.6+0.4*sin(iTime*6.0)); }
  }

  float pres = smoothstep(0.0, 1.0, clamp(uPresence, 0.0, 1.0));
  // THE ENTRANCE IS A FADE, not a portal. The black-hole aperture (and the emergence travel
  // that sold it) is gone: the being simply fades in at its resting place, slowly, and fades
  // out quickly. fullHole is pinned at 0 - the realms keep their portal-coupled code paths
  // (realm 1's lensing/shockwave, realm 3's stir), which simply go inert.
  ...
  float breathe = 1.0 + (0.018 + 0.022*tempo)*sin(iTime*0.9) + 0.030*uAudio.y + 0.02*uBeat + 0.090*voice;
  float scaleFull = 0.225*breathe*(1.0 + gSwell)*(1.0 - 0.05*fatigue)*(1.0 + 0.025*soc);
  ...
  vec2  portalPos = vec2(-0.18, 0.10);
  vec2 centre = centreFull;
  ...
  if (disc > 0.0) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      q.xy = rot2(vort)*q.xy;
      float rotA = t*(0.08 + 0.05*float(j)) + q.y*0.35
                 + sin(iTime*1.7 + q.y*2.0)*(0.20*irr + gChurn*0.15);
      q.xy = rot2(rotA)*q.xy;
    ...
    float heartBreath = 0.30 + 0.22*puls + 0.80*voice + 0.5*uBeat;
    heartBreath *= 1.0 - fatigue*0.30*(0.5 + 0.5*sin(iTime*6.7 + sin(iTime*2.9)*2.5));
    ...
    inner += mix(hot, vec3(0.55,0.90,1.00), 0.5)*arcs*(0.10 + 0.18*lum + 0.25*excite);
    ...
    float mAmp = smoothstep(0.0, 0.1, mProg)*smoothstep(1.0, 0.85, mProg)
               *(0.25 + 0.75*excite + 1.2*uBeat);
    inner += mix(vec3(0.80,0.94,1.00), heartC, 0.4)*head*mAmp*1.6;

    // PARTICLES: excitement lofts twinkles; anger ejects embers
    float pAmt = max(excite, irr)*(1.0 - 0.9*fatigue);
    if (pAmt > 0.01) {
      vec3 partCol = vec3(0.0);
      for (int k = 0; k < 12; k++) {
        float seed = float(k)*7.31 + 2.7;
        float rnd  = hash13(vec3(seed, seed*1.7, seed*2.3));
        float rate = (0.08 + rnd*0.06)*(1.0 + 1.6*irr);
        float cyc  = t*rate + rnd*9.0;
        float prog = fract(cyc);
        float gen  = floor(cyc);
        float ang  = hash13(vec3(seed, gen, seed + gen*0.61))*6.28318;
        float curl = (1.0 - irr)*prog*2.2;
        vec2  pp   = nuc + vec2(cos(ang + curl), sin(ang + curl))*(0.15 + prog*0.72)
                   + vec2(0.0, (1.0 - irr)*prog*0.22);
        float fade = smoothstep(0.0, 0.15, prog)*smoothstep(1.0, 0.60, prog);
        float twk  = 0.7 + 0.3*sin(iTime*11.0 + seed*5.0);
        float pd   = length(sp - pp);
        vec3  pc   = mix(vec3(0.75,0.92,1.00)*1.8, vec3(1.00,0.30,0.55)*2.4, irr);
        float pGlow = exp(-pd*pd/mix(0.00065, 0.00042, irr));
        float pCore = exp(-pd*pd/mix(0.000045, 0.000028, irr));
        partCol   += pc*(pGlow*0.22 + pCore*1.65)*fade*twk;
      }
      inner += partCol*pAmt*(0.4 + 0.6*lum);
    }
  ...
  //   - portalPos/fullHole: fullHole is PINNED AT 0 (the portal effect is retired - the entrance
  //     is a fade), so any realm code coupling to it is inert but harmless. portalPos remains as
  //     a place (realm 1's room is oriented around it).
  ...
  // THE TWO BLACK HOLES (RETIRED). The portal effect is gone: the entrance is a fade, so
  // fullHole/dockHole are pinned at 0 and this block never runs. It is kept (gated, inert) in
  // case a future realm wants an aperture back.
  ...
  // CLOCKBAR SUMMON. Nothing remains here at rest. Hover condenses a wide, soft cloud of the same
  // electric blue as the idle interior, with living filaments rather than a button-shaped fill.
  float barMistA = 0.0;
  if (uFill > 0.5 && uHover > 0.001) {
    vec2 mq = (uv - uCentreDock)/vec2(0.050, 0.017);
    float mr2 = dot(mq,mq);
    float veil = exp(-mr2*1.25);
    float cloud = vnoise(vec3(mq*1.65 + vec2(t*0.10,-t*0.06), t*0.11));
    float folds = pow(ridge(vec3(mq*2.8, t*0.24 + cloud*1.7)), 5.0);
    float pulse = 0.84 + 0.16*sin(iTime*2.1);
    float h = smoothstep(0.0, 1.0, uHover);
    barMistA = clamp((veil*(0.18 + 0.34*cloud) + folds*veil*0.34)*h, 0.0, 0.78);
    vec3 mistC = mix(vec3(0.02,0.18,0.72), vec3(0.20,0.78,1.00),
                     clamp(cloud*0.75 + folds*0.55,0.0,1.0));
    outCol += mistC*(barMistA*1.55 + folds*veil*h*0.50)*pulse;
  }
  ...
  float scaleFull = 0.205*breathe*(1.0 + gSwell)*(1.0 - 0.05*fatigue);
