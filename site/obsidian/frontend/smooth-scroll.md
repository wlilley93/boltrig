...
Lenis hijacks the **global** wheel (`smoothWheel: true`), so a nested element with
`overflow-y-auto` will *not* scroll natively — the wheel event is consumed by
Lenis for page scroll (which does nothing on a single-viewport page). Add
**`data-lenis-prevent`** to the scrollable element so Lenis skips it and lets the
browser scroll it normally. Pair it with `overscroll-contain` to stop scroll
chaining at its edges.

```tsx
<div data-lenis-prevent className="overflow-y-auto overscroll-contain">…</div>
```

Used by the particle-brain parameters drawer ([[particle-brain]] → `BrainControls`).
