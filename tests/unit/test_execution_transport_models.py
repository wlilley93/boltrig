...
def test_event_payload_is_exact_bounded_sensitive_free_and_normalized() -> None:
    ...
    too_many = (
        b'{"references":[' + b",".join(b'"id"' for _ in range(MAX_COLLECTION_ITEMS + 1)) + b"]}"
    )
...
def test_canonical_payload_rejects_normalized_sensitive_keys_at_any_depth(key: str) -> None:
    with pytest.raises(ValueError, match="sensitive key|controls"):
        CanonicalPayload._from_mapping({"outer": [{key: "forbidden"}]})


@pytest.mark.parametrize("key", ("unsafe\nkey", "safe\u202etoken", "safe\u2066key"))
def test_canonical_payload_rejects_control_characters_in_structural_keys(key: str) -> None:
    with pytest.raises(ValueError, match="controls"):
        CanonicalPayload._from_mapping({key: "forbidden"})


def test_canonical_payload_preserves_newlines_and_tabs_in_text_values() -> None:
    payload = CanonicalPayload._from_mapping({"message": "line one\n\tline two"})

    assert payload.to_mapping() == {"message": "line one\n\tline two"}
