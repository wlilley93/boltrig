...
def _wf(steps: list[dict]) -> WorkflowDefinition:
    return WorkflowDefinition(
        ...
        definition={"name": "cf", "version": "1", "steps": steps},
        intent_tags=[],
    )
...
async def test_branch_runs_only_the_matching_arm():
    ...
    wf = _wf(
        [
            {"id": "gate", "parents": [], "action": "ticket.create", "params": {"title": "gate"}},
            {
                "id": "cond",
                "parents": ["gate"],
                "action": "flow.branch",
...
async def test_loop_binding_substitutes_typed_item_and_index():
    ...
    store.set_tenant_permissions(TenantPermissions(T, GrantSet.of(["*"])))
    k = Kernel(store)
    await k.register_adapter(T, _TypedAdapter())
    lib = WorkflowLibrary(k.store, executor=LocalDurableExecutor(), kernel=k)
    wf = _wf(
        [
            {
                ...
                "parents": [],
                "action": "flow.loop",
                "params": {"items": [{"title": "alpha"}, {"title": "beta"}]},
            },
            {
                "id": "make",
                "parents": ["loop"],
    ...
    record = await lib.execute(T, wf.id, {}, _ctx())
    by_id = {s["id"]: s for s in record["steps"]}
    assert by_id["loop"]["output"]["count"] == 2
    assert by_id["make"]["status"] == "ok"
    assert by_id["make"]["output"]["iterations"] == [
        {"payload": {"title": "alpha"}, "position": 0},
        {"payload": {"title": "beta"}, "position": 1},
    ]
    assert record["status"] == "completed"
...
async def test_loop_with_dynamic_items_from_parent_output():
    k = await _kernel()
    lib = WorkflowLibrary(k.store, executor=LocalDurableExecutor(), kernel=k)
    # The entry step exposes a stable input list; the loop may only reference
    # an ancestor output and binds each value through the ordinary verb schema.
    wf = _wf(
        [
            {"id": "start", "parents": [], "action": "trigger.start", "params": {}},
            {
                ...
                "parents": ["start"],
                "action": "flow.loop",
                "params": {"items_from": "$start.output.inputs.tags"},
            },
            {
                "id": "body",
                "parents": ["loop"],
    ...
    await lib.register(wf)
    record = await lib.execute(T, wf.id, {"tags": ["one", "two"]}, _ctx())
    by_id = {s["id"]: s for s in record["steps"]}
    assert by_id["loop"]["output"]["count"] == 2
    assert by_id["body"]["status"] == "ok"
    assert [item["title"] for item in by_id["body"]["output"]["iterations"]] == ["one", "two"]
    assert record["status"] == "completed"
...
@pytest.mark.invariant("FR-CTL-03")
async def test_loop_bindings_enter_the_ordinary_strict_verb_schema():
    # Binding is whole-value substitution before dispatch; the kernel still
    # enforces the target verb's closed schema without hidden metadata.
    from boltrig.adapters.base import Result, VerbSpec
    ...
    wf = _wf(
        [
            {"id": "loop", "parents": [], "action": "flow.loop", "params": {"items": ["a", "b"]}},
            {
                "id": "echo",
                "parents": ["loop"],
                "action": "strict.echo",
                "params": {"msg": None},
                "loop_bindings": {"msg": "item"},
            },
        ]
    )
    await lib.register(wf)
    ...
    assert by_id["echo"]["output"]["iterations"] == [
        {"echo": {"msg": "a"}},
        {"echo": {"msg": "b"}},
    ]
    assert record["status"] == "completed"
...
@pytest.mark.invariant("FR-WFL-19")
async def test_empty_loop_dispatches_no_body_actions():
    k = await _kernel()
    lib = WorkflowLibrary(k.store, executor=LocalDurableExecutor(), kernel=k)
    wf = _wf(
        [
            {"id": "loop", "parents": [], "action": "flow.loop", "params": {"items": []}},
            {
                "id": "body",
                "parents": ["loop"],
                "action": "ticket.create",
    ...
    by_id = {step["id"]: step for step in record["steps"]}
    assert record["status"] == "completed"
    assert by_id["loop"]["output"]["count"] == 0
    assert by_id["body"]["status"] == "ok"
    assert by_id["body"]["output"] == {"iterations": [], "count": 0}


@pytest.mark.invariant("FR-WFL-19")
async def test_invalid_loop_contract_dispatches_nothing():
    from boltrig.adapters.base import Result, VerbSpec
    ...
    wf = _wf(
        [
            {"id": "would-run", "action": "count.go", "params": {}},
    ...
    by_id = {step["id"]: step for step in record["steps"]}
    assert by_id["body"]["reason"] == "loop_binding_target_missing"
    assert by_id["would-run"]["reason"] == "invalid_loop_contract"
