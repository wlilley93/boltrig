...
from boltrig.models import Channel, ChannelBinding, GrantSet, User
...
# --------------------------------------------------------------------------- #
# The grant ceiling is derived, never the bare wildcard (SEC-34 parity with PAT)
# --------------------------------------------------------------------------- #
@pytest.mark.security
@pytest.mark.invariant("SEC-01")
async def test_member_binding_gets_no_wildcard_ceiling():
    # a bare subject bound as member: operates, but never configures/administers
    s = await _store_with_binding("member")
    p = await resolve_channel_principal(s, _channel(), "U-ext-42")
    assert p is not None
    assert p.grants != GrantSet.of(["*"])  # the wildcard ceiling is gone
    assert p.grants.permits("ticket.read")  # operate
    assert not p.grants.permits("control.channel.connect")  # never administer


@pytest.mark.security
@pytest.mark.invariant("SEC-34")
async def test_provisioned_user_current_grants_cap_the_channel_identity():
    # the subject's user record is authoritative (US-USR-03): a binding minted at
    # the owner tier still resolves to the user's CURRENT recorded grants only.
    s = await _store_with_binding("superadmin")
    await s.upsert_user(User(
        id="alice", tenant_id=T, role="member",
        scope={"verbs": ["ticket.read"]}, status="active",
    ))
    p = await resolve_channel_principal(s, _channel(), "U-ext-42")
    assert p is not None
    assert p.grants.permits("ticket.read")
    assert not p.grants.permits("ticket.create")  # the user does not hold it
    assert p.scope == {"verbs": ["ticket.read"]}


@pytest.mark.security
@pytest.mark.invariant("SEC-34")
async def test_binding_tier_clamps_a_more_privileged_user():
    # the composition only ever narrows (COUNTY 5): a member binding over a
    # superadmin user cannot administer through the channel.
    s = await _store_with_binding("member")
    await s.upsert_user(User(
        id="alice", tenant_id=T, role="superadmin", scope={"all": True}, status="active",
    ))
    p = await resolve_channel_principal(s, _channel(), "U-ext-42")
    assert p is not None
    assert p.grants.permits("ticket.create")
    assert not p.grants.permits("control.workspace.update")


@pytest.mark.security
@pytest.mark.invariant("SEC-34")
async def test_deactivated_users_channel_identity_dies_with_user():
    # a deactivated user's channel binding stops working at once (SEC-34), the
    # same way their PATs do.
    s = await _store_with_binding("member")
    await s.upsert_user(User(
        id="alice", tenant_id=T, role="member", scope={"all": True}, status="deactivated",
    ))
    assert await resolve_channel_principal(s, _channel(), "U-ext-42") is None
