...
/**
 * Debris clustering. The film's sphere is not statistically uniform -- it has
 * clusters and voids -- and no per-particle hash can produce that, because a
 * hash has no neighbourhoods. Two interfering low-frequency waves do: their
 * product makes blobs, and squaring sharpens the blobs into clumps. The weight
 * is renormalised so the MEAN brightness holds and only the variance rises --
