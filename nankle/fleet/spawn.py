...
from nankle.adapters.base import AdapterError, ErrorClass, Result
...
if TYPE_CHECKING:  # type-only: keeps fleet import independent of fastapi/kernel
    ...
    from nankle.kernel.app import Principal, SpawnBody
    from nankle.kernel.dispatch import AgentInvoker
    from nankle.models import ModelEndpoint


# --- local error types (created here, not in the frozen models layer) ---------
class SkillNotFound(NankleError):
...
class NoCapableRuntime(NankleError):
...
def make_app_spawner(
    ...
    propagate so ``create_app``'s NankleError handler maps them to status codes.
