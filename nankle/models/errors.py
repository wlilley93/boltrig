...
class SchemaValidationError(NankleError):
...
class BindingNotFound(NankleError):
...
class GrantMissing(NankleError):
...
class TenantIsolation(NankleError):
...
class BudgetExceeded(NankleError):
...
class DepthExceeded(NankleError):
...
class ContextRequirementsUnmet(NankleError):
...
class CredentialResolution(NankleError):
...
class SensitiveDataMisrouted(NankleError):
...
class DegradedMode(NankleError):
