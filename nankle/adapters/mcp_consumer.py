"""Consume an external MCP server as a Nankle adapter (Round Two, US-MCP-03).
...
injected for tests (and to let Nankle consume its own MCP face).
"""
...
class McpConsumerAdapter:
    ...
    async def execute(
        ...
        nankle = result.get("_nankle") or {}
        if result.get("isError"):
            return Result.failure(
                AdapterError(ErrorClass.INVALID, nankle.get("reason") or "mcp tool error")
            )
        return Result.success(nankle.get("output") or {})
    ...
    async def _call(self, request: dict) -> dict:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            r = await client.post(
                self._url, json=request, headers={"x-nankle-mcp-token": self._token or ""}
            )
            return r.json()
