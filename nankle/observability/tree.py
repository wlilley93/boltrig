...
async def build_tree(store: Store, tenant_id: str, root_run_id: str) -> dict[str, Any]:
    ...
    def assemble(run_id: str) -> dict[str, Any]:
        node = dict(runs.get(run_id, {"run_id": run_id, "actions": 0}))
        node["children"] = [assemble(c) for c in children.get(run_id, [])]
