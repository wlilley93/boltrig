...
def install_security(app: FastAPI, *, env: dict | None = None) -> None:
    ...
    Config (env, all optional):
      NANKLE_ALLOWED_HOSTS   - comma list for Host validation (default '*' off only
                               in dev; a deployment SHOULD set it).
      NANKLE_CORS_ORIGINS    - comma list of allowed browser origins (default none:
                               same-origin only; never '*').
      NANKLE_MAX_BODY_BYTES  - request-body cap (default 1 MiB).
    """
    e = env if env is not None else os.environ
    hosts = _csv(e.get("NANKLE_ALLOWED_HOSTS")) or ["*"]
    origins = _csv(e.get("NANKLE_CORS_ORIGINS"))
    try:
        max_body = int(e.get("NANKLE_MAX_BODY_BYTES") or _DEFAULT_MAX_BODY)
    except ValueError:
    ...
    if origins:
        ...
        app.add_middleware(
            ...
            allow_headers=["authorization", "content-type", "x-nankle-mcp-token"],
        )
