...
def register_access_routes(app, *, principal_dep, get_kernel) -> None:
    ...
    async def mint_my_token(body: dict, k=K, p=P) -> JSONResponse:
        from nankle.identity.provisioning import ensure_user_record
        from nankle.identity.tokens import mint_pat
    ...
    async def my_connections(request: Request, p=P) -> dict:
        ...
        return {
            ...
            "snippets": {
                "claude_code": (
                    f"claude mcp add --transport http nankle {mcp} "
    ...
    async def create_invite(body: dict, k=K, p=P) -> JSONResponse:
        from nankle.identity.tokens import bounded_expiry
